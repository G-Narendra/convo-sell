...
import { Send, Utensils, BrainCircuit, Sparkles, ChefHat, Mic, MicOff, Volume2 } from "lucide-react";
...
export default function Home() {
  ...
  const toggleListening = () => {
    ...
    if (isListening) {
      ...
      window.speechSynthesis.cancel(); // Stop AI talking if user starts
      try {
      ...
      <div className="w-1/4 max-w-sm border-r border-neutral-800 bg-neutral-900/50 p-6 flex flex-col justify-between hidden md:flex">
        <div>
          <div className="flex items-center gap-3 mb-10">
            ...
            <h1 className="text-2xl font-bold tracking-tight">Global Hub<span className="text-orange-400">@MDX</span></h1>
          </div>
            ...
            <div className="bg-neutral-800/50 p-5 rounded-2xl border border-neutral-800">
              ...
              <div className="space-y-3 mt-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-medium px-2 py-1 rounded-md flex items-center gap-1 ${isVoiceMode ? "bg-blue-500/20 text-blue-400" : "bg-neutral-800 text-neutral-300"}`}>
                  ...
                  <ul className="space-y-2 mb-4">
                    {kitchenOrder.items?.map((item: any, idx: number) => (
                      <li key={idx} className="text-sm flex flex-col">
                        ...
                        {item.notes && <span className="text-xs text-neutral-500 italic pl-4">Note: {item.notes}</span>}
                      </li>
                    ))}
                  </ul>
                  <div className="text-xs text-neutral-500 pt-3 border-t border-orange-500/20">
                    Spice: {kitchenOrder.preferences?.spice_level || "Standard"} | Alerts: {kitchenOrder.preferences?.allergies?.join(", ") || "None"}
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
      ...
      <div className="flex-1 flex flex-col relative bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-neutral-900 via-neutral-950 to-black">
          ...
          {processedMessages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-lg mx-auto opacity-70">
              ...
              <p className="text-neutral-400 mb-8">
                Your AI host for Middlesex University Dubai. Tell us what you're craving, or just let us know how your day is going!
              </p>
              
              <div className="flex gap-4">
                <button 
                  onClick={() => setIsVoiceMode(false)}
                  className={`px-6 py-3 rounded-full font-medium transition-all ${!isVoiceMode ? 'bg-orange-500 text-black shadow-lg shadow-orange-500/20' : 'bg-neutral-800 text-neutral-300'}`}
                >
                  Type Order
                </button>
                <button 
                  onClick={() => setIsVoiceMode(true)}
                  className={`px-6 py-3 rounded-full font-medium flex items-center gap-2 transition-all ${isVoiceMode ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/20' : 'bg-neutral-800 text-neutral-300'}`}
                >
                ...
                >
                  <div className={`max-w-[85%] md:max-w-[70%] p-4 rounded-3xl ${
                    msg.role === "user" 
                      ...
                      : "bg-orange-950/30 border border-orange-500/20 text-neutral-200 rounded-bl-sm"
                  }`}>
                    {msg.role === "assistant" && (
        ...
        <div className="p-4 md:p-6 bg-gradient-to-t from-neutral-950 via-neutral-950/80 to-transparent">
          ...
          >
            ...
            <div className="flex items-center justify-between px-2">
              <button 
                ...
                className={`text-xs font-medium px-3 py-1.5 rounded-full flex items-center gap-1.5 transition-colors ${isVoiceMode ? 'bg-blue-500/20 text-blue-400' : 'bg-neutral-800 text-neutral-400 hover:bg-neutral-700'}`}
              >
          ...
          <div className="text-center mt-4 text-xs text-neutral-600">
            {kitchenOrder ? "Kitchen staff preparing your order." : "Using AI sentiment analysis to find exactly what hits the spot."}
          </div>
        </div>
      </div>
    </div>
